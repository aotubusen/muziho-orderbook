...
class OrderTest{
    ...
    fun testCreateOfferOrder(){
        val order = Order(1, 100.00, OrderSide.O, 2)
        assertEquals(1, order.id)
        assertEquals(100.00, order.price, 0.00)
        assertEquals(OrderSide.O, order.side)
        assertEquals(2, order.size)


        val order2 = Order(2, 100.00, OrderSide.B, 2)
        assertEquals(2, order2.id)
        assertEquals(100.00, order2.price, 0.00)
        assertEquals(OrderSide.B, order2.side)
    ...
    fun testCopyOrderAndUpdate(){
        ...
        val orderB = Order(2, 100.00, OrderSide.B, 2)
